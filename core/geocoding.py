...
# A region can plausibly span up to ~2500km (Russia alone is huge), but a
# "city" result more than that from its region's centroid is not a
# legitimate match -- it's almost certainly a mis-extracted/ambiguous name
# that Nominatim's global search happened to resolve somewhere unrelated.
MAX_PLAUSIBLE_DISTANCE_KM = 2500
...
def enrich_item_with_city(item: dict, known_regions: list[str], primary_region: str = None) -> dict:
    """
    ...
    for candidate in candidates:
        ...
        if primary_region and not is_plausible_for_region(coords[0], coords[1], primary_region):
            print(f"  [!] Rejected geocoding: '{candidate}' resolved to a location implausibly "
                  f"far from '{primary_region}' -- falling back to region centroid instead.")
            continue
